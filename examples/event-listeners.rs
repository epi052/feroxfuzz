...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let words = Wordlist::with_words(["stuff", "things"])
        .name("words")
        .build();

    let methods = HttpMethodsCorpus::safe().name("methods").build();
    ...
    let request = Request::from_url(
        "http://localhost:8000/",
        Some(&[
            ShouldFuzz::URLParameterValue(b"admin=FUZZ", b"="),
    ...
    let decider = StatusCodeDecider::new(200, |status, observed, state| {
        if observed >= status && observed < 300 {
            state
                .events()
                .notify(&Event::Message("[MESSAGE] found 200 response".to_string()));
            state.events().notify(&MyCustomEventType {
    ...
    fn test_fn(event: &Event) {
        if let Event::Message(message) = event {
            println!("Hello, event world! => {}", message);
        }
    }

    struct MyCustomEventType {
        message: String,
        status_code: u16,
        action: Action,
    }

    state.events().subscribe(test_fn);

    state.events().subscribe(|event: &MyCustomEventType| {
        println!(
            "Hello, custom event world! => {} - {} - {:?}",
            event.message, event.status_code, event.action
        );
    });
